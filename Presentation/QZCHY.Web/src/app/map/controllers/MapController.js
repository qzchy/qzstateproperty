...
app.controller('MapCtrl', ['$window', '$scope', '$timeout','$filter', '$uibModal', 'MapService', 'PropertyService', 'GovernmentService',
    ...
    $scope.setClusterData = function () {
        ...
        angular.forEach($scope.properties, function (property, v) {
            ...
            var constructColor =  "red";
            var landColor ="orange";
            var landUnderColor = "purple";

            if ($scope.mapType == 1) {
                switch (property.next) {
                    case 0:
                        constructColor = "red";
                        landColor = "red";
                        landUnderColor = "red";
        ...
        else
        {
            switch (type) {
                ...
                case 1:
                    ...
                    $scope.search.params.nextStep.jk = true;
                    break;
                case 2:
                    //拍卖处置
                    ...
                    $scope.search.params.nextStep.jk = false;
                    break;
                case 3:
                    //国有收储
                    $scope.search.params.nextStep.auction = false;
